<title>UI & UX designer in indore | Graphic designer in indore</title>
<meta name="keywords" content="UI & UX designer in indore | Graphic designer in indore
">

<meta name="description" content="Mitsafe aims to meet the demands of today's companies and organizations with designs that put people first and are pleasant to interact with. Branding, responsive web design, mobile app design, user experience counseling, and promotional designs are just some of the UI/UX services we offer.">
<?php include('layout/header.php');?>



    <!--Contents-->
    <main class="services-page style-5">


    
        <!-- ====== end features ====== -->

        <!-- ====== start services ====== -->
        <section class="services section-padding bg-white pb-50 style-6">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-lg-8">
                        <div class="section-head text-center mb-60 style-5">
                            <h1 class="mb-20">UI/UX Design services

</span> </h1>
                            <p>
                            Design is the primary thing to see on a website. There has been a rise in the number of developers and designers focusing on user interface and user experience design, or frontend development.




                            </p>
                        </div>
                    </div>
                </div>
            </div>
            
        </section>
        <!-- ====== end services ====== -->


        <!-- ====== start about ====== -->
        <section class="about pt-0 pb-150 style-5" style="margin-top:-200px!important">
            <div class="content">
                <div class="container">
                    <div class="row align-items-center">
                        <div class="col-lg-4 order-2 order-lg-0">
                            <div class="section-head mb-30 style-5">
                                <h2>Our <span>Services </span></h2>
                            </div>
                            <p>  With our services, we can show you what your app or website will look like in its entirety, down to the text, images, and buttons, so you can plan out the layout and design before developing it.


</p>
<p>We turn your ideas and thoughts into a design that is clear and fully functional. We make user interfaces that match what the user wants and what they can understand.
</p>
                           
                        </div>
                        <div class="col-lg-8 order-0 order-lg-2">
                            <div class="img main-img1">
                                <!-- <img src="" alt="" class="sm-circle">
                                <img src="" alt="" class="img-body"> -->
                                <img src="assets/uiux/first.gif" alt="" class="img-body" >
                               
                                <!-- <img src="assets/img/about/about_s5_1_5.png" alt="" class="lg-circle">  -->
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="content" style="margin-top:-200px!important">
                <div class="container">
                    <div class="row align-items-center">
                        <div class="col-lg-6">
                            <div class="img main-img2">
                                <img src="assets/img/about/about_s5_2_1.png" alt="">
                                <img src="assets/uiux/second.gif"  alt="" class="img-body">
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="section-head mb-30 style-5">
                                <h2>UI/UX Design services in Indore at Mitsafe
</span> </h2>
                            </div>
                            <p>
                            Mitsafe knows the value of a clean and good-looking website. We aim to meet the demands of today's companies and organizations with designs that put people first and are pleasant to interact with. Branding, responsive web design, mobile app design, user experience counseling, and promotional designs are just some of the UI/UX services we offer.


                            </p>
                            
                        </div>
                    </div>
                </div>
            </div>
            <div class="content pb-0" style="margin-top:-200px!important">
                <div class="container">
                    <div class="row align-items-center">
                        <div class="col-lg-6 order-2 order-lg-0">
                            <div class="section-head mb-30 style-5">
                                <h2>UI/UX designer company in Indore</span> </h2>
                            </div>
                            <p>
                            Mitsafe is a UI/UX designer company in Indore and offers services worldwide, Our designers combine technology and creativity to create dynamic and responsive designs that are compatible with the platform. With the extensive experience of 7 years and are known for our creativity and hands-on experience in all areas of UI/UX. We build user-friendly designs so that users can navigate your website easily and in turn, the conversion rate increases. 

  </p>
                            
                        </div>
                        <div class="col-lg-6 order-0 order-lg-2">
                            <div class="img">
                                <img src="assets/uiux/third.gif" alt="" class="img-body">
                               
                                
                            </div>
                        </div>
                    </div>
                </div>
            </div>
           

            




















        </section>
        <!-- ====== end about ====== -->


        <!-- ====== start testimonials ====== -->
        <section class="testimonials section-padding bg-gray5 style-5">
            <div class="container">
                <div class="section-head text-center mb-60 style-5">
                    <h2 class="mb-20"> Clients <span> Reviews </span> </h2>
                    <p>ALL TYPES OF IT SOLUTION @ MODERN TECHNOLOGY FOR BUILDING THE MODERN. </p>
                </div>
            </div>
            <div class="content">
                <div class="testimonial-slider position-relative style-5">
                    <div class="swiper-container pb-70">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <a href="#" class="testi-card style-5">
                                    <div class="stars">
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                    </div>
                                    <div class="text" style="font-size: 12px;">
                                        “Techiesys us a place where you can learn high level digital marketing and professional skills in website design and development online marketing also leadership qualities and team work and good working environment.”
                                    </div>
                                    <div class="user mt-40 text-center">
                                        <div class="icon-80 rounded-circle img-cover overflow-hidden m-auto">
                                            <img src="http://mitsafe.com/assets/images/testimonial/avatar/2.jpg" alt="">
                                        </div>
                                        <h6>Mike Hotten</h6>
                                        <small>CEO, Brick Consulting</small>
                                    </div>
                                </a>
                            </div>
                            <div class="swiper-slide">
                                <a href="#" class="testi-card style-5">
                                    <div class="stars">
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                    </div>
                                    <div class="text" style="font-size: 12px;">
                                        “  A fairly decent company providing services like web development, SEO, digital marketing , etc. Know a few of their clients. Most of them spoke well about the company. However, like any business, there is scope for improvements.”
                                    </div>
                                    <div class="user mt-40 text-center">
                                        <div class="icon-80 rounded-circle img-cover overflow-hidden m-auto">
                                            <img src="http://mitsafe.com/assets/images/testimonial/avatar/1.jpg" alt="">
                                        </div>
                                        <h6> Monty Moni</h6>
                                        <small>CEO, Keen IT Solution</small>
                                    </div>
                                </a>
                            </div>
                            <div class="swiper-slide">
                                <a href="#" class="testi-card style-5">
                                    <div class="stars">
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                        <i class="fas fa-star"></i>
                                    </div>
                                    <div class="text" style="font-size: 12px;">
                                        “I just want to thank you for the excellent job you have done on my website. The service was incredible and responsive. Developers are very supportive also they were treated as the valued customer, Affordable price compared to the others web development.”
                                    </div>
                                    <div class="user mt-40 text-center">
                                        <div class="icon-80 rounded-circle img-cover overflow-hidden m-auto">
                                            <img src="http://mitsafe.com/assets/images/testimonial/avatar/3.jpg" alt="">
                                        </div>
                                        <h6>Nancy Lowe</h6>
                                        <small>CEO, Brick Consulting </small>
                                    </div>
                                </a>
                            </div>
                           
                        </div>
                    </div>
                    <div class="swiper-pagination"></div>
                </div>
            </div>
        </section>
        <!-- ====== end testimonials ====== -->

<!-- 
        <!-- ====== start pricing ====== -->
        <!-- <section class="pricing section-padding border-bottom border-1 brd-gray style-5">
            <div class="container">
                <div class="section-head text-center mb-60 style-5">
                    <h2 class="mb-20"> Choose Plan and <span> Kick-start </span> </h2>
                    <p>
                        Whether you’re just getting started with rapid testing or scaling across the organization, we’ve
                        got you covered.
                    </p>
                </div>
                <div class="pricing-tabsHead text-center">
                    <div class="price-radios">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="inlineRadioOptions" id="monthly-input" value="option1">
                            <label class="form-check-label" for="monthly-input">
                                Billed Monthly
                            </label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="inlineRadioOptions" id="yearly-input" value="option2" checked>
                            <label class="form-check-label" for="yearly-input">
                                Billed yearly 
                                <small class="alert-danger text-danger rounded-pill ms-1">-15%</small>
                            </label>
                        </div>
                    </div>
                </div>
                <div class="table-responsive">
                    <div class="content">
                        <div class="price-head">
                            <div class="price-headTitle">
                                <img src="assets/img/icons/price_s5.png" alt="">
                            </div>
                            <div class="price-headItem">
                                <h6>basic</h6>
                                <h2 class="monthly_price">Free <span>/mo</span></h2>
                                <h2 class="yearly_price">Free <span>/yr</span></h2>
                                <small>Free forever.!</small>
                                <small>No credit card required.</small>
                            </div>
                            <div class="price-headItem bg-gray5">
                                <h6>Premium</h6>
                                <h2 class="monthly_price color-blue5">$49 <span>/mo</span></h2>
                                <h2 class="yearly_price color-blue5">$49 <span>/yr</span></h2>
                                <small>Free forever.!</small>
                                <small>No credit card required.</small>
                                <div class="label">
                                    best choice
                                </div>
                            </div>
                            <div class="price-headItem">
                                <h6>ultimate</h6>
                                <h2 class="monthly_price">$99 <span>/mo</span></h2>
                                <h2 class="yearly_price">$99 <span>/yr</span></h2>
                                <small>Free forever.!</small>
                                <small>No credit card required.</small>
                            </div>
                        </div>

                        <div class="price-body">
                            <div class="price-bodyItems">
                                <div class="price-bodyTitle">
                                    Unlimited Projects
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                                <div class="price-item bg-gray5">
                                    <i class="bi bi-check2"></i>
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                            </div>

                            <div class="price-bodyItems">
                                <div class="price-bodyTitle">
                                    Lifetime Support & SEO Experts Consult
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                                <div class="price-item bg-gray5">
                                    <i class="bi bi-check2"></i>
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                            </div>

                            <div class="price-bodyItems">
                                <div class="price-bodyTitle">
                                    Advance Dashboard
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                                <div class="price-item bg-gray5">
                                    <i class="bi bi-check2"></i>
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                            </div>

                            <div class="price-bodyItems">
                                <div class="price-bodyTitle">
                                    Data Security & Backup
                                </div>
                                <div class="price-item">
                                    <span>200 GB</span>
                                </div>
                                <div class="price-item bg-gray5">
                                    <span>1 TB</span>
                                </div>
                                <div class="price-item">
                                    <span>Unlimited</span>
                                </div>
                            </div>

                            <div class="price-bodyItems">
                                <div class="price-bodyTitle">
                                    Cloud Hosting & Domain
                                </div>
                                <div class="price-item">
                                </div>
                                <div class="price-item bg-gray5">
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                            </div>

                            <div class="price-bodyItems">
                                <div class="price-bodyTitle">
                                    Free Google Analysis <i class="fas fa-info-circle"></i>
                                </div>
                                <div class="price-item">
                                    <span>$5/Year</span>
                                </div>
                                <div class="price-item bg-gray5">
                                    <i class="bi bi-check2"></i>
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                            </div>

                            <div class="price-bodyItems">
                                <div class="price-bodyTitle">
                                    Multi-Vendor Marketplace Support
                                </div>
                                <div class="price-item">
                                </div>
                                <div class="price-item bg-gray5">
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                            </div>

                            <div class="price-bodyItems">
                                <div class="price-bodyTitle">
                                    Seller App
                                </div>
                                <div class="price-item">
                                </div>
                                <div class="price-item bg-gray5">
                                    <i class="bi bi-check2"></i>
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                            </div>

                            <div class="price-bodyItems">
                                <div class="price-bodyTitle">
                                    Auto Lifetime Upgrade <i class="fas fa-info-circle"></i>
                                </div>
                                <div class="price-item">
                                </div>
                                <div class="price-item bg-gray5">
                                    <i class="bi bi-check2"></i>
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                            </div>

                            <div class="price-bodyItems">
                                <div class="price-bodyTitle">
                                    Free SSL Certificate <i class="fas fa-info-circle"></i>
                                </div>
                                <div class="price-item">
                                </div>
                                <div class="price-item bg-gray5">
                                </div>
                                <div class="price-item">
                                    <i class="bi bi-check2"></i>
                                </div>
                            </div>
                        </div>

                        <div class="price-foot">
                            <div class="price-footTitle">
                            </div>
                            <div class="price-footItem">
                                <a href="page-contact-5.html" class="btn rounded-pill blue5-3Dbutn hover-blue2 sm-butn fw-bold">
                                    <span> Get Started Now </span>
                                </a>
                            </div>
                            <div class="price-footItem bg-gray5">
                                <a href="page-contact-5.html" class="btn rounded-pill blue5-3Dbutn hover-blue2 sm-butn fw-bold">
                                    <span> Get Started Now </span>
                                </a>
                            </div>
                            <div class="price-footItem">
                                <a href="page-contact-5.html" class="btn rounded-pill blue5-3Dbutn hover-blue2 sm-butn fw-bold">
                                    <span> Get Started Now </span>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>  -->
        <!-- ====== end pricing ====== -->


    </main>
    <!--End-Contents-->

    <!-- ====== start footer ====== -->
  <?php include('layout/innerFooter.php')?>
  <script src="assets/js/lib/jquery-3.0.0.min.js"></script>
<script src="assets/js/lib/jquery-migrate-3.0.0.min.js"></script>
<script src="assets/js/lib/bootstrap.bundle.min.js"></script>
<script src="assets/js/lib/wow.min.js"></script>
<script src="assets/js/lib/jquery.fancybox.js"></script>
<script src="assets/js/lib/lity.js"></script>
<script src="assets/js/lib/swiper.min.js"></script>
<script src="assets/js/lib/jquery.waypoints.min.js"></script>
<script src="assets/js/lib/jquery.counterup.js"></script>
<script src="assets/js/lib/pace.js"></script>
<script src="assets/js/lib/scrollIt.min.js"></script>
<script src="assets/js/main.js"></script>
</body>

</html>